    ...
    return {
      ...
      columns: [
        ...
        { label: "Responses", field: "responses", type: "number" },
        {
          ...
          field: "apdex_score",
          type: "number",
          formatFn: this.formatToFixed2,
          tdClass: this.tdClassApdex
        },
        {
          label: "Errors",
          field: "errors",
          type: "number",
          tdClass: this.tdClassErrors
        },
        {
          label: "Req rate",
          field: "req_rate",
        ...
        { label: "Redirect", field: "redirect", type: "number" },
        {
          ...
          field: "client_error",
          type: "number",
          tdClass: this.tdClassCErr
        },
        {
          label: "Server Error",
          field: "server_error",
          type: "number",
          tdClass: this.tdClassSErr
        },
        ...
        {
          label: "Avg Req Size",
          field: "avg_req_clength",
          type: "number",
          formatFn: this.formatToFixed0
        },
        {
          label: "Avg Res Size",
          field: "avg_res_clength",
          type: "number",
          formatFn: this.formatToFixed0
        }
      ],
  ...
  methods: {
    ...
    updateStats: function() {
      ...
      this.dbdata.setWData("w1", {
        value: pathOr(0, ["method", "GET", "requests"], statsContainer),
        trend: sampleTrend,
        total: totalRequests
      });
      this.dbdata.setWData("w2", {
        value: pathOr(0, ["method", "POST", "requests"], statsContainer),
        trend: sampleTrend,
        total: totalRequests
      });
      this.dbdata.setWData("w3", {
        value: pathOr(0, ["method", "PUT", "requests"], statsContainer),
        total: totalRequests
      });
      this.dbdata.setWData("w4", {
        value: pathOr(0, ["method", "DELETE", "requests"], statsContainer),
        trend: sampleTrend,
        total: totalRequests
      });
      ...
      this.dbdata["w23"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].requests
      );
      this.dbdata.touch("w23");

      this.dbdata["w24"].data.labels = allMethods;
      this.dbdata["w24"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].errors
      );
      this.dbdata.touch("w24");

      this.dbdata["w25"].data.labels = allMethods;
      this.dbdata["w25"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].apdex_score
      );
      this.dbdata.touch("w25");

      this.dbdata["w26"].data.labels = allMethods;
      this.dbdata["w26"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].req_rate
      );
      this.dbdata.touch("w26");

      this.dbdata["w27"].data.labels = allMethods;
      this.dbdata["w27"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].err_rate
      );
      this.dbdata.touch("w27");

      this.dbdata["w28"].data.labels = allMethods;
      this.dbdata["w28"].data.datasets[0].data = allMethods.map(
        x => statsContainer.method[x].avg_time
      );
