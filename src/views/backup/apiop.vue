    ...
    <div class="row items-center" style="margin-bottom: 10px;">
      <div class="col-md-12 q-ma-sm">
        ...
        <div class="text-subtitle1">
          Check out
          <a href="https://github.com/slanatech/swagger-stats" target="_blank"
            >swagger-stats</a
          >
          project to see this dashboard in action
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    initialize: function() {
      ...
      this.dbdata.setWData("w25", {
        data: { labels: [], datasets: [{ data: [] }] }
      });
      this.dbdata.setWData("w26", {
        data: { labels: [], datasets: [{ data: [] }] }
      });
      this.dbdata.setWData("w27", {
        data: { labels: [], datasets: [{ data: [] }] }
      });
      this.dbdata.setWData("w28", {
        data: { labels: [], datasets: [{ data: [] }] }
      });
    ...
    updateStats: function() {
      ...
      // Update dashboard
      let apiOpData = pathOr(
        null,
        ["apiop", this.apiOpPath, this.apiOpMethod],
        statsContainer
      );
      if (!apiOpData) {
        return;
      ...
      this.dbdata.setWData("w2", {
        value: pathOr(0, ["apdex_score"], apiOpStats),
        trend: apdexTrend
      });
      this.dbdata.setWData("w3", {
        value: pathOr(0, ["req_rate"], apiOpStats),
        trend: sampleTrend
      });
      this.dbdata.setWData("w4", {
        value: pathOr(0, ["err_rate"], apiOpStats),
        trend: sampleTrend
      });
      this.dbdata.setWData("w5", {
        value: pathOr(0, ["avg_time"], apiOpStats),
        trend: sampleTrend,
        trendMax: 100
      });
      let { value, qualifier } = utils.formatBytes(
        pathOr(0, ["avg_req_clength"], apiOpStats),
        2
      );
      ...
      this.dbdata.setWData("w7", {
        value: pathOr(0, ["errors"], apiOpStats),
        total: requestsTotal,
        trend: sampleTrend
      });
      this.dbdata.setWData("w8", {
        value: pathOr(0, ["success"], apiOpStats),
        total: requestsTotal,
        trend: sampleTrend
      });
      this.dbdata.setWData("w9", {
        value: pathOr(0, ["redirect"], apiOpStats),
        total: requestsTotal
      });
      this.dbdata.setWData("w10", {
        value: pathOr(0, ["client_error"], apiOpStats),
        total: requestsTotal,
        trend: sampleTrend
      });
      this.dbdata.setWData("w11", {
        value: pathOr(0, ["server_error"], apiOpStats),
        total: requestsTotal,
        trend: sampleTrend
      });
      ({ value, qualifier } = utils.formatBytes(
        pathOr(0, ["avg_res_clength"], apiOpStats),
        2
      ));
      this.dbdata.setWData("w12", {
        value: value,
        qualifier: qualifier,
        trend: sampleTrend,
        trendMax: 30
      ...
      this.dbdata["w25"].data.labels = utils.bucketsToLabels(
        pathOr([], ["duration", "buckets"], apiOpDetails)
      );
      this.dbdata["w25"].data.datasets[0].data = pathOr(
        [],
        ["duration", "values"],
        apiOpDetails
      );
      ...
      this.dbdata["w26"].data.labels = utils.bucketsToLabels(
        pathOr([], ["req_size", "buckets"], apiOpDetails)
      );
      this.dbdata["w26"].data.datasets[0].data = pathOr(
        [],
        ["req_size", "values"],
        apiOpDetails
      );
      ...
      this.dbdata["w27"].data.labels = utils.bucketsToLabels(
        pathOr([], ["res_size", "buckets"], apiOpDetails)
      );
      this.dbdata["w27"].data.datasets[0].data = pathOr(
        [],
        ["res_size", "values"],
        apiOpDetails
      );
      ...
      this.dbdata["w28"].data.datasets[0].data = codes.map(x =>
        pathOr(0, ["code", x, "count"], apiOpDetails)
      );
      this.dbdata.touch("w28");

      let paramNames = Object.keys(pathOr({}, ["parameters"], apiOpDetails));
      this.dbdata["w29"].data = paramNames.map(x =>
        pathOr(0, ["parameters", x], apiOpDetails)
      );
