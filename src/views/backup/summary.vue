  ...
  methods: {
    ...
    updateStats: function() {
      ...
      for (let entry of timelineSorted) {
        ...
        asData.push([
          new Date(entry.ts),
          pathOr(0, ["stats", "apdex_score"], entry)
        ]);
      ...
      this.dbdata.setWData("w2", {
        value: pathOr(0, ["all", "apdex_score"], statsContainer),
        trend: trendsData[1]
      });
      ...
      this.dbdata.setWData("w4", {
        value: errRate,
        trend: trendsData[3],
        trendMax: reqRateTrendMax
      });
      this.dbdata.setWData("w5", {
        value: pathOr(0, ["sys", "cpu"], statsContainer),
        trend: trendsData[4]
      });
      let { value, qualifier } = utils.formatBytes(
        pathOr(0, ["sys", "heapUsed"], statsContainer),
        2
      );
      ...
      this.dbdata.setWData("w12", {
        value: pathOr(0, ["sys", "lag"], statsContainer),
        trend: trendsData[11]
      });
